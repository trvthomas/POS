...
<body onload="getStats()">
	...
	<div class="contentBox">
		<h3 class="is-size-5">Sales Statistics</h3>
		<p>Observe the growth of your business through monthly and daily sales statistics</p>
		...
		<div class="box">
			...
			<div class="tabs is-centered is-boxed">
				<ul>
					<li class="is-active"><a href="/trv/admin/statistics.php"><span class="icon is-small"><i class="fas fa-coins"></i></span><span>Total Sales</span></a></li>
					...
					<li><a href="/trv/admin/statistics-products.php"><span class="icon is-small"><i class="fas fa-tshirt"></i></span><span>Sold Quantities</span></a></li>
				...
				<div class="column">
					<div class="field">
						<div>
							...
							<label class="label" for="checkboxCash">Show cash sales</label>
						</div>

						<div>
							<input type="checkbox" class="is-checkradio" id="checkboxCard" onclick="setFilters()">
							<label class="label" for="checkboxCard">Show card sales</label>
						</div>
						...
						<div <?php if ($metodoPagoPersonalizado == "") {
							...
							<label class="label" for="checkboxOther">Show <?php echo $metodoPagoPersonalizado; ?> sales</label>
						</div>
					</div>
				</div>
		...
		<div class="columns is-multiline is-centered has-text-left">
			<div class="column is-one-third">
				<div class="box is-shadowless">
					...
					<h3 class="is-size-3"><i class="fas fa-receipt fa-fw"></i> <span id="boxesNumberSales">...</span></h3>
				</div>
			</div>
	...
	<form method="POST" action="/trv/admin/include/get-stats-sales.php" style="display: none" id="getStatsForm" onsubmit="return getStatsReturn();">
	...
	<script>
		...
		function getStatsReturn() {
			$.ajax({
				type: 'POST',
				url: '/trv/admin/include/get-stats-sales.php',
				...
				success: function(response) {
					if (response['errores'] == true) {
						...
						document.getElementById('chartShow').innerHTML = "<p class='has-text-centered is-size-5'><b>No results found</b></p>";
					} else if (response['tabla_ventas'] != "") {
						var tableFinal = "<tr><td>Date</td>";

						if (showCash == true) {
							tableFinal += "<td>Cash sales</td>";
						}
						if (showCard == true) {
							tableFinal += "<td>Card sales</td>";
						}
						if (showOther == true) {
							tableFinal += "<td><?php echo $metodoPagoPersonalizado; ?> sales</td>";
						}

						tableFinal += "<td>Total income</td></tr>" + response['tabla_ventas'];

						document.getElementById('chartTable').innerHTML = tableFinal;

						document.getElementById('boxesNumberSales').innerHTML = thousands_separators(response['stats_numero_ventas']);
