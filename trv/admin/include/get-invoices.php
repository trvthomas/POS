<?php include_once $_SERVER['DOCUMENT_ROOT'] . "/trv/admin/include/verifySession.php";
	...
	$bgColor = "";
	if($row["cancelada"] == true){ $bgColor = "has-background-danger-light"; }
	
	$pluralAddS = "s";
	if(count($decoded) == 1){ $pluralAddS = ""; }
	
	$paymentBgColor = "pastel-bg-green";
	if($row["formaPago"] == "Tarjeta"){ $paymentBgColor = "pastel-bg-purple"; }
	else if($row["formaPago"] == "Multipago"){ $paymentBgColor = "pastel-bg-darkorange"; }
	else if($row["formaPago"] != "Efectivo"){ $paymentBgColor = "pastel-bg-cyan"; }
