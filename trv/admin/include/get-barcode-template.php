<?php include_once $_SERVER['DOCUMENT_ROOT'] . "/trv/admin/include/verifySession.php";
	...
	if($_POST["getTemplateShowTime"] == "true"){ $plantillaCodigos .= '<p class= "has-text-centered">Generado el: <b>' . date("d-m-Y h:i a") . '</b></p>'; $plantillaCodigosPDF.= '<p class= "has-text-centered">Generado el: <b>' . date("d-m-Y h:i a") . '</b></p>'; }
	
	$plantillaCodigos .= '<hr><div class= "columns is-multiline is-centered">';
	$plantillaCodigosPDF .= '<hr><table>';
	
	while($row = $result->fetch_assoc()){
	if($plantillaCodigosPDFNum == 5){ $plantillaCodigosPDFNum = 1; }
	if($plantillaCodigosPDFNum == 1){ $plantillaCodigosPDF .= "<tr>"; }
	...
	if($row["variable_price"] == 1){ $finalPrice = 'Precio variable'; }
	...
	if($plantillaCodigosPDFNum == 5){ $plantillaCodigosPDF .= "</tr>"; }
	}
