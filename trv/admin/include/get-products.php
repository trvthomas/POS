<?php include_once $_SERVER['DOCUMENT_ROOT'] . "/trv/admin/include/verifySession.php";
	...
	if($valorRentabilidad == 0){ $valorRentabilidad = $row["precio"]; }
	...
	if($valorCant <= 0){ $colorUtil = "#ef4d4d"; }
	if($rentabilidad <= 0){ $colorRent = "#ef4d4d"; }
	
	if($row["imagen"] != ""){ $imageProduct = $row["imagen"]; }
