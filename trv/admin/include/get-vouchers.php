<?php include_once $_SERVER['DOCUMENT_ROOT'] . "/trv/admin/include/verifySession.php";
	...
	$bgColor = "";
	if($row["totalAvailable"] <= 0){ $bgColor = 'has-background-warning-light" title= "Bono utilizado en su totalidad"'; }
	else if(strtotime($row["expiration"]) < strtotime(date("Y-m-d"))){ $bgColor = 'has-background-danger-light" title= "Bono vencido"'; }
